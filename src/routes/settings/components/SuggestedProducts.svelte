<script lang="ts">
    ...
    const Init_SetIems = () => {
        if ($_store.load_settings) {
            suggested_products = $_store.site_settings.find(s => s.id == 'suggested_products')?.data
        }
    }
    ...
    $: selectedSuggestedProd = (products.find((p) => p.name == name_suggested_prod)?.name ?? "Добавить товар")
...
<div class="w-full my-8">
    <p class="font-bold mb-2 scroll-mt-[70px]" id="suggested_products">Дополнительные товары {city_name}</p>
    <Popover.Root bind:open={open.suggested_products} let:ids>
                    ...
                    <Command.Group>
                        {#each products as p}
                            <Command.Item
                                ...
                                if (!suggested_products.find(i => i.id == p.id && i.city == city)) {
                                    edit_site_settings('suggested_products', { id: p.id, city })
                                } else {
                            ...
                            >
                                <Check class={cn( "mr-2 h-4 w-4",  name_suggested_prod !== p.data.name && "text-transparent")}  />
                                {p.data.name}
                            </Command.Item>
                        {/each}
                    </Command.Group>
    ...
    <div class="grid grid-cols-3 mt-5">
        ...
        {#if suggested_products_city.length > 0}
            ...
            {@const globalIndex = suggested_products.findIndex(item => item.id === p.id && item.city == city)}
                <div class="flex items-center space-x-3">
                    ...
                    <Dialog.Root>
                        ...
                        <Dialog.Content class="sm:max-w-[425px]">
                            <Dialog.Header>
                                <Dialog.Title>Вы уверены, что хотите удалить доп. товар {product?.prod.name || '-'}?</Dialog.Title>
                              ...
                              <Dialog.Close class="w-full">
                                <Button 
                                    ...
                                    on:click={()=> {
                                        if (globalIndex !== -1) {
                                            edit_site_settings('suggested_products', null, globalIndex)
                                            toast.success("Успешно!", {
                                                description: `Доп. товар "${product?.prod.name || '-'}" удален`,
                                                action: { label: "Закрыть", onClick: () => console.info("") },
                                                position: 'top-right',
                                            })
                                        } else {
